...
# root.geometry("+%d+%d" % (x, y))
    ...
    def table_win():
        ...
        processes_string = prcs.get()  # Assuming prcs.get() returns '2,3,4,7,7'
        processes = list(map(int, processes_string.split(',')))
        ...
        for i in range(1, len(processes) + 1):  # Rows
        ...
        sub = Button(newwin, text="Submit", command=submit).grid(
            row=num_processes+2, column=1)
        Reset = Button(newwin, text="Reset", command=reset).grid(
            row=num_processes+3, column=1)
